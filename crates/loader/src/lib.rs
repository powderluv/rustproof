...
//! loader — a minimal *static* ELF64 loader.
//!
//! [`load_elf`] parses an ET_EXEC x86-64 ELF64 image and maps every `PT_LOAD`
//! segment into a caller-supplied [`vspace::AddressSpace`], allocating leaf frames
//! from an [`abi::FrameAllocator`]. Each segment is materialized page-by-page at
//! 4 KiB granularity: a fresh frame is zeroed, the file-backed bytes are copied in,
//! and the tail past `p_filesz` stays zero (`.bss`). No dynamic linking, no
//! relocations, no interpreter — the entry point is returned verbatim.
//!
//! Frame contents are written through the same physical→virtual window `vspace`
//! uses to reach page-table frames: byte `b` of physical frame `f` lives at host
//! virtual address `f + aspace.phys_offset()` (identity when `phys_offset == 0`).

use abi::{FrameAllocator, VirtAddr, PAGE_SIZE};
use vspace::{AddressSpace, PageFlags};
...
pub fn load_elf(
    ...
    if rd_u16(elf, OFF_E_MACHINE)? != EM_X86_64 {
...
fn load_segment(
    ...
    // Leaf permissions: user code, always present; writable per PF_W; and mark
    // non-executable unless PF_X is set (W^X-friendly — NX stays off for code).
    let mut flags = PageFlags::PRESENT | PageFlags::USER;
    if p_flags & PF_W != 0 {
        flags = flags | PageFlags::WRITABLE;
    }
    if p_flags & PF_X == 0 {
        flags = flags | PageFlags::NO_EXEC;
    }
    ...
    while page_va < seg_end_va {
        ...
        // window `vspace` uses to reach page-table frames. We form a slice of
...
mod tests {
    ...
    // ---- mock frame allocator + address space (mirrors vspace's own tests) ----
    ...
    /// Build a valid one-`PT_LOAD` ET_EXEC x86-64 ELF64 image with `code` placed at
    ...
    fn make_elf(entry: u64, seg: &Seg, code: &[u8]) -> Vec<u8> {
        ...
        v[OFF_E_MACHINE..OFF_E_MACHINE + 2].copy_from_slice(&EM_X86_64.to_le_bytes());
    ...
    fn rejects_bad_machine() {
        let mut img = valid_image();
        img[OFF_E_MACHINE..OFF_E_MACHINE + 2].copy_from_slice(&0x28u16.to_le_bytes()); // ARM
        let (mut fa, mut aspace) = fresh_space();
        assert_eq!(
            load_elf(&img, &mut aspace, &mut fa),
    ...
    fn reports_map_failure_when_frames_exhausted() {
        // create() takes 1 frame (PML4); a two-page segment needs 2 data frames +
        // intermediate tables. Capacity 2 leaves one frame after the PML4: it
        // becomes the first data page, then AddressSpace::map runs out of frames
        // for an intermediate table → MapFailed.
        let seg = Seg {
            vaddr: 0x40_0000,
            offset: 0x1000,
            filesz: 4,
    ...
    fn reports_out_of_frames_for_data_page() {
        // Only the PML4 frame exists (capacity 1); the first data-page allocation
