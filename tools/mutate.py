...
MUTATIONS = [
    ...
    ("kernel: an I/O page-table leaf ignores WRITE (a device gets more than granted)",
     ...
     "        pte |= IW;\n        pte\n    }", "rig-iommu"),
    ("kernel: withdrawal leaves the I/O entry PRESENT (device keeps its reach)",
     "crates/kernel/src/lib.rs",
     "            core::ptr::write_volatile(at as *mut u64, 0);\n            any = true;",
     "            any = true;", "rig-iommu"),
    ("kernel: MAP_DMA honours a capability naming a domain that does not exist",
     "crates/kernel/src/lib.rs",
     ...
     "    let di = 0;\n    let Some(l1) = domain_l1(domain) else {\n        return abi::syserr::NO_MEM;\n    };\n    let Some(ri) = region_index(region_id) else {\n        return abi::syserr::NO_CAP;\n    };\n    let npages", "rig-iommu"),
    ("kernel: MAP_DMA hands out reach on a machine with no IOMMU to bound it",
     "crates/kernel/src/lib.rs",
     "    let ids = domain_ids();\n    if ids.iter().all(|&id| id == 0) {\n        return abi::syserr::NO_MEM;\n    }\n    let Some(di) = domain_lookup(&ids, domain) else {\n        return abi::syserr::NO_CAP;\n    };\n    let Some(l1)",
     "    let ids = domain_ids();\n    if false {\n        return abi::syserr::NO_MEM;\n    }\n    let Some(di) = domain_lookup(&ids, domain) else {\n        return abi::syserr::NO_CAP;\n    };\n    let Some(l1)", "rig"),
    ("kernel: withdrawing a region from a domain skips the hardware",
     "crates/kernel/src/lib.rs",
     "        // Hardware first — the model is the index that finds the hardware.\n        any |= clear_io_mappings_in(di, pfn);",
     "        any |= false;", "rig-iommu"),
    ("kernel: exit withdraws no DMA reach at all",
     "crates/kernel/src/lib.rs",
