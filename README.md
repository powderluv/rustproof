...
**Status:** early M0. The nucleus **boots and handles CPU exceptions** — a minimal image comes up under QEMU via the PVH protocol, switches to 64-bit long mode, brings up COM1 serial, installs an IDT, and dumps a clean register/frame trace on any CPU fault (a forced `#PF` shows the faulting `CR2`) — task T0.1. Not yet: a physical frame allocator, address spaces, the GPU driver, or any verification. No in-house proof engineer, so the verification track (M1+) is unstaffed until that gate is restructured — see [`docs/ai-proof-writer.md`](docs/ai-proof-writer.md). The near-term work is ordinary (hard) systems work: grow the nucleus, then get the untrusted driver to dispatch one wave on real hardware.
