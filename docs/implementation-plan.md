...
- **T0.8 (in-guest MES/IH bring-up), ~4–10 weeks, uncertain** — the system-RAM ring state does not survive the no-FLR handover, so the guest must re-establish MES/KIQ/IH (today Python). Decide **before T0.7** whether to port `bringup.py` to C++/Rust (recommended; keeps CPython out of the guest) or run it under a CPython personality (larger surface) — it changes T0.5's syscall surface.

**Honest M0 budget:** the mandatory path sums to **~33–63 engineer-weeks ≈ 8–15 months solo**; overlap (T0.1–T0.3 vs. an early strace-driven T0.5 spike; T0.4 vs. T0.6) pulls it toward **~6–12 months**. Do not present a shorter number. This is dominated by GPU bring-up and C++ hosting, not by the microkernel.
...
**Gate G1 — the proof-engineer gate (hard, blocks all of Track V).** There is no in-house proof engineer today. M0 is reachable by the systems engineer alone; **M1 and everything above are unstaffed and therefore unscheduled until this gate is cleared.** Do not put M1–M5 dates on any roadmap before then. Clear it by either:
- (a) **hiring** a Verus/SMT-experienced proof engineer (small pool; overlaps CMU/Parno, MSR/Hawblitzel-Lattuada lineage, Utah/Mars Research — expect a long search), **or**
- (b) signing a **funded research collaboration** with Mars Research (Anton Burtsev, University of Utah) — the Atmosphere authors, who built the exact flat-map/ghost-subtree page-table proof technique this plan leans on. This is the fastest realistic path and buys the head start of their proofs.

The **gate condition**: begin M1 proof work only when (a) or (b) holds. Use the **Atmosphere-reproduction exercise** ([`verification.md`](verification.md) §7) as both the hiring test and the ramp deliverable, so the first month of paid proof work also de-risks the technique on our pinned toolchain. (A strong Rust systems engineer can reach Verus productivity in ~3–6 months via this ramp if grown in-house — budget it as real headcount, not slack.)
...
5. **Spike the bootable nucleus (T0.1 start).** PVH ELF note → long-mode trampoline → serial (`uart_16550` on `0x3F8`) → GDT/IDT with a double-fault IST → paging from the PVH memory map → LAPIC/TSC timer → bitmap frame allocator with a separate DMA-capable pool. Acceptance: `qemu-system-x86_64 -enable-kvm -kernel nucleus.elf -serial mon:stdio` prints boot progress and a forced page-fault dumps cleanly.
